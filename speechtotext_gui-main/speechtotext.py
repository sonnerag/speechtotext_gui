...
transcription_label = ttk.Label(transcription_tab, text="Transcription:", font=("Arial", 18))
transcription_label.pack(pady=10)
transcription_text = scrolledtext.ScrolledText(transcription_tab, wrap=tk.WORD, width=100, height=20, font=("Arial", 12))
transcription_text.pack(pady=10, padx=15, fill=tk.BOTH, expand=True)

# Tab 2: Translation
translation_tab = ttk.Frame(notebook)
notebook.add(translation_tab, text="Translation")

translation_label = ttk.Label(translation_tab, text="Translation:", font=("Arial", 18))
translation_label.pack(pady=10)
translation_text = scrolledtext.ScrolledText(translation_tab, wrap=tk.WORD, width=100, height=20, font=("Arial", 12))
translation_text.pack(pady=10, padx=15, fill=tk.BOTH, expand=True)

# Tab 3: Summary
summary_tab = ttk.Frame(notebook)
notebook.add(summary_tab, text="Summary")

summary_label = ttk.Label(summary_tab, text="Summary:", font=("Arial", 18))
summary_label.pack(pady=10)
summary_text = scrolledtext.ScrolledText(summary_tab, wrap=tk.WORD, width=100, height=10, font=("Arial", 12))
summary_text.pack(pady=10, padx=15, fill=tk.BOTH, expand=True)

# Tab 4: Keywords
keywords_tab = ttk.Frame(notebook)
notebook.add(keywords_tab, text="Keywords")

keywords_label = ttk.Label(keywords_tab, text="Keywords:", font=("Arial", 18))
keywords_label.pack(pady=10)
keywords_text = scrolledtext.ScrolledText(keywords_tab, wrap=tk.WORD, width=100, height=10, font=("Arial", 12))
keywords_text.pack(pady=10, padx=15, fill=tk.BOTH, expand=True)

# Tab 5: Action Items
action_items_tab = ttk.Frame(notebook)
notebook.add(action_items_tab, text="Action Items")

action_items_label = ttk.Label(action_items_tab, text="Action Items:", font=("Arial", 18))
action_items_label.pack(pady=10)
action_items_text = scrolledtext.ScrolledText(action_items_tab, wrap=tk.WORD, width=100, height=10, font=("Arial", 12))
action_items_text.pack(pady=10, padx=15, fill=tk.BOTH, expand=True)

# Tab 6: Sentiment Analysis
sentiment_tab = ttk.Frame(notebook)
notebook.add(sentiment_tab, text="Sentiment Analysis")

sentiment_label = ttk.Label(sentiment_tab, text="Sentiment:", font=("Arial", 18))
...
style.configure("Start.TButton", font=("Arial", 12), foreground="black", background="lightgray", padding=5)
style.map("Start.TButton", background=[("active", "gray")])
style.configure("Stop.TButton", font=("Arial", 12), foreground="black", background="red", padding=5)
style.map("Stop.TButton", background=[("active", "darkred")])

# Toggle recording function
recording = False  # State to track if recording is active

def toggle_recording():
    global recording
    if not recording:
        # Start recording
        record_button.config(text="Stop Recording", style="Stop.TButton")
        Thread(target=stream_transcribe).start()
    else:
        # Stop recording
        record_button.config(text="Start Recording", style="Start.TButton")
        stop_recording()
...
upload_button = ttk.Button(buttons_frame, text="Upload Audio File", command=upload_audio_file, style="Custom.TButton")
upload_button.grid(row=0, column=1, padx=5)

# Export transcription button
export_button = ttk.Button(buttons_frame, text="Export Transcription", command=export_transcription, style="Custom.TButton")
